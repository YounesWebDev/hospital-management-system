...
import { Search, ReceiptText, Clock, User } from 'lucide-react';
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="space-y-1">
                        <h1 className="text-3xl font-semibold tracking-tight">Billing Overview</h1>
                        <p className="text-muted-foreground">Manage all patient billing records and tracking payment statuses.</p>
                    </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {filtered.length > 0 ? (
                                ...
                                <CardContent className="space-y-4 p-4 pt-0">
                                    ...
                                    <div className="pt-3 flex justify-end border-t border-border/50">
                                        <Button asChild variant="outline" size="sm" className="h-7 px-2 text-[11px]">
                                            <Link href={billing.show({ billing: record.id }).url}>
                                                View Detail
                                            </Link>
                                        </Button>
                                    </div>
                        ...
                        <div className="col-span-full text-center py-12 border rounded-lg bg-muted/20">
                            <ReceiptText className="mx-auto size-12 text-muted-foreground/50 mb-3" />
                            <p className="text-muted-foreground">No billing records found matching your search.</p>
                        </div>
                    )}
                </div>
            </div>
