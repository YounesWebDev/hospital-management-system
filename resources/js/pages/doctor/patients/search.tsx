...
import { Search, Stethoscope, UserRoundSearch } from 'lucide-react';
                        ...
                        <div className="max-w-2xl space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="max-w-xl text-sm leading-6 text-muted-foreground md:text-base">
                                    Search the patient registry, open a medical file, and move directly into notes,
                                    prescriptions, analyses, and billing context.
                                </p>
                            </div>
                        </div>

                        <div className="grid gap-3 sm:grid-cols-3">
                            {statEntries.slice(0, 3).map(([label, value]) => (
                                <div key={label} className="rounded-lg border border-border/70 bg-background/90 p-4 shadow-sm">
                ...
                <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
                    <Card className="rounded-lg border-border/70 bg-card/95 shadow-sm">
                        <CardHeader className="border-b border-border/70 bg-muted/35">
                            ...
                            <CardDescription>
                                Search by patient code, name, or username. Results update through a normal Inertia visit.
                            </CardDescription>
                        </CardHeader>
                                ...
                                {({ processing }) => (
                                    <>
                                        <div className="flex flex-col gap-3 md:flex-row">
                                            ...
                                            <Button type="submit" disabled={processing} className="h-11 rounded-lg px-6">
                                        ...
                                        {patients.length === 0 ? (
                                            <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-border/70 bg-muted/20 px-6 py-14 text-center">
                                                ...
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-sm text-muted-foreground">
                                                        Try a different patient code, name, or username.
                                                    </p>
                                                </div>
                                            </div>
                                        ) : (
                                            <div className="grid gap-4 md:grid-cols-2">
                                                {patients.map((patient) => (
                                                    <Card key={patient.id} className="rounded-lg border-border/70 bg-background/80 shadow-sm">
                    ...
                    <div className="space-y-6">
                        <Card className="rounded-lg border-border/70 bg-card/95 shadow-sm">
                            <CardHeader>
                                <CardTitle className="text-base">Clinical overview</CardTitle>
                                <CardDescription>
                                    The doctor workspace keeps the patient search and file review in one clean flow.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4 text-sm text-muted-foreground">
                                {statEntries.slice(3).map(([label, value]) => (
                                    <div key={label} className="rounded-lg border border-border/70 bg-muted/35 p-4">
