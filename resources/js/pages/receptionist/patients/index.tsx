...
import { Search, User, Mail, Fingerprint, Plus } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { create, show } from '@/routes/receptionist/patients';
type Patient = {
  id: number;
  code: string;
  name: string;
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            <h1 className="text-3xl font-semibold tracking-tight">Patient Registration</h1>
            <p className="text-muted-foreground">Manage and register new patients into the hospital system.</p>
          </div>
          <Button asChild className="w-fit">
            <Link href={create.url()}>
              <Plus className="mr-2 size-4" /> Register New Patient
            </Link>
          </Button>
        </div>
